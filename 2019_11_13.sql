...
--fk 제약을 dept_test.deptno컬럼을 참조하도록 생성하여
--1번 이외의 부서번호는 emp_test 테이블에 입력 될 수 없다.

--emp_test fk 테스트 insert
INSERT INTO emp_test VALUES (9999, 'brown', 1);

--2번 부서는 dept_test테이블에 존재하지 않는 데이터 이기 떄문에
--fk제약에 의해 INSERT가 정상적으로 동작하지 않는다.
...
CREATE TABLE emp_test (
    empno NUMBER(4),
    ename VARCHAR2(1),
    --dept_test.deptno커럼에 인덱스가 없기 때문에 정상적으로
    --fk 제약을 생성할 수 없다.
    deptno NUMBER(2) REFERENCES dept_test(deptno)
);
...
--삭제 가능하다
--emp_test.deptno컬럼이 dept_test.deptno컬럼을 참조하기 때문에
--emp_test.deptno컬럼을 먼저 삭제해야 dept_test.deptno컬럼을 삭제할 수 있다. 
DELETE emp_test WHERE empno = 9999;
DELETE dept_test WHERE deptno = 1;

--FK 제약 옵션
--default : 데이터 입력/삭제시 순차적으로 처리해줘야 fk 제약을 위해
--          하지 않는다.
...
--FK제약 default 옵션시에는 부모 테이블의 데이터를 삭제하기 전에 자식 테이블에서
--참조하는 데이터가 없어야 정상적으로 삭제가 가능했음
--ON DELETE CASCADE의 경우 부모 테이블 삭제시 참조하는 자식 테이블의 데이터의
--참조 컬럼을 NULL로 수정한다.
--같이 삭제한다.
--1.삭제 쿼리가 정상적으로 실행 되는지?
--2.자식 테이블에 데이터가 삭제 되었는지?
...
--데이터는 복제하지 않고 특정 테이블의 컬럼 형식만 가져올순 없을가?
